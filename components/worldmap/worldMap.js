...
export default class WorldMap extends Component {
  ...
  componentDidMount() {
    ...
    window.addEventListener('mousemove', function (event) {
      // Update the mouse vector with the mouse position
      ...
      // If the intersection includes the circleMarker, show the tooltip
      for (var i = 0; i < intersects.length; i++) {
        if (intersects[i].object === circleMarker) {
      ...
      // If the intersection does not include the circleMarker, hide the tooltip
      tooltip.style.visibility = 'hidden';
    }, false);
